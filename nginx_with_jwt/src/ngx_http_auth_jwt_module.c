...
static ngx_int_t ngx_http_auth_jwt_handler(ngx_http_request_t *r)
{
	...
	if (exp < now)
	{
		 ...
		 * Globenet dbg info
		 **/
		ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "DEBUG INFO: now time: %d", now);
		ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "DEBUG INFO: jwtCookieValChrPtr: %d", jwtCookieValChrPtr);
		ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "DEBUG INFO: &jwt: %d", &jwt);
		ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "DEBUG INFO: jwt : %d", jwt);
		ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "DEBUG INFO: exp time: %d", exp);
